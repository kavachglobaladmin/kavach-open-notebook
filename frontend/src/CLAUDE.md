...
Next.js React application providing UI for Open i_Notes research assistant. Three-layer architecture: **pages** (Next.js App Router), **components** (feature-specific UI), and **lib** (data fetching, state management, utilities).
...
- `(dashboard)/`: Protected routes (i_Notes, sources, search, models, etc.)
...
- **source**, **i_Notes**, **search**, **podcasts**: Feature-specific components consuming hooks
...
- **Resource modules** (`sources.ts`, `chat.ts`, `i_Notes.ts`, etc.): Endpoint-specific functions returning typed responses
- **Pattern**: All requests go through `apiClient`; auth token auto-added from localStorage

#### `lib/hooks/` — React Query + Custom Logic
- **Query hooks**: `usei_Notesources`, `useSources`, `useSource` — TanStack Query wrappers with cache keys
- **Mutation hooks**: `useCreateSource`, `useUpdateSource`, `useDeleteSource` — mutations with toast feedback + cache invalidation
- **Complex hooks**: `usei_NotesChat`, `useSourceChat` — session management, message streaming, context building
...
- API request/response shapes, domain models (i_Notes, Source, Note, etc.)
...
- **Pattern**: Components call `useTranslation()` hook; access strings via `t.common.save`, `t.i_Notes.title`

## Data & Control Flow Walkthrough

### Example: i_Notes Chat
1. **Page** (`i_Notes/[id]/page.tsx`) fetches initial data, passes `i_NotesId` to `ChatColumn` component
2. **Hook call** (`usei_NotesChat()`):
   - Queries sessions for i_Notes via TanStack Query
...
5. **Related queries** auto-refetch: i_Notes, sources list, etc.

## Key Patterns & Cross-Layer Coordination

### Caching & Invalidation
- **Query keys**: `QUERY_KEYS.i_Notes(id)`, `QUERY_KEYS.sources(i_NotesId)` — hierarchical structure
- **Broad invalidation**: `['sources']` invalidates all source queries; trade-off between accuracy + performance
- **Auto-refetch**: `refetchOnWindowFocus: true` on frequently-changing data (sources, i_Notes)

### Auth & Protected Routes
- **Proxy** (`src/proxy.ts`): Redirects root `/` to `/i_Notes`
- **Auth store**: Validates token via `/i_Notes` API call (actual validation, not JWT decode)
...
- **Context**: Modals pass data (e.g., i_Notes ID) to child components
...
- **Feature folders** (`source/`, `i_Notes/`, `podcasts/`): Group related components
