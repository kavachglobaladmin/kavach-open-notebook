...
import { useSources, useAddSourcesToi_Notes } from '@/lib/hooks/use-sources'
...
export function AddExistingSourceDialog({
  ...
  // Get sources already in this i_Notes
  const { data: currenti_Notesources } = useSources(i_NotesId)
  const currentSourceIds = useMemo(
    () => new Set(currenti_Notesources?.map(s => s.id) || []),
    [currenti_Notesources]
  )
