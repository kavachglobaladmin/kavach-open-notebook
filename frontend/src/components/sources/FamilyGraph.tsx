    ...
    >
      ...
      >
        ...
        {nodes.map(node => {
          ...
          // Always open to the RIGHT of the circle, vertically centered with it
          // If circle is on the far right, open to the left instead
          const spaceRight = size.w - (node.x + MEMBER_R)
          const openRight = spaceRight >= DETAIL_W + 20

          const popupX = openRight
            ? node.x + MEMBER_R + 16
            : node.x - MEMBER_R - 16 - DETAIL_W

          // Vertically centered with the circle center
          ...
          return (
            ...
            >
              {/* Name + relation — image style: large bold name, small relation below */}
              <p className="text-[17px] font-extrabold text-slate-900 leading-tight mb-0.5">{node.name}</p>
              <p className="text-[12px] text-slate-400 capitalize font-medium mb-3">{node.relation}</p>
              <div className="h-px bg-slate-100 mb-3" />
              {/* Details as plain text lines — image style */}
              {node.details.length > 0 ? (
                <div className="space-y-2.5">
                  {node.details.map((d: string, i: number) => {
                    ...
                    return (
                      <p key={i} className="text-[13px] text-slate-700 leading-snug">
                        {key
