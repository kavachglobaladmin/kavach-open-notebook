...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLocalLoading(true)
    const backendUser = await loginUserBackend(normalizedEmail, password)
    if (backendUser) saveUserLocally({ name: backendUser.name, email: normalizedEmail, password })
    else {
      const localUser = findUserLocally(normalizedEmail, password)
      if (!localUser) { setLocalError('Invalid email or password.'); setLocalLoading(false); return }
    }
    localStorage.setItem('kavach_current_user', normalizedEmail)
    // Store password in sessionStorage so it survives page reload within the
    // same tab — the auth-store restores apiPassword from here on rehydration.
    // sessionStorage is cleared when the tab is closed, so it never persists
    // across browser sessions (unlike localStorage).
