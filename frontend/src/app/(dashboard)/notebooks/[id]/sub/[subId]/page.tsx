...
/**
 * Sub-folder notebook page — /notebooks/[id]/sub/[subId]
 *
 * Renders the full original notebook view (Sources | Notes | Chat) for a
 * sub-folder notebook, identical to what the top-level notebook page used to
 * show before the folder hierarchy was introduced.
 *
 * The only difference from the original is the back-navigation link, which
 * goes to the parent folder (/notebooks/[id]) instead of /notebooks.
 */
...
export default function SubFolderNotebookPage() {
  const { t } = useTranslation()
  const params = useParams()
  const searchParams = useSearchParams()

  // Parent notebook short ID (used for back navigation)
  ...
  // Sub-folder (child) notebook full SurrealDB ID
  const rawSubParam = params?.subId ? decodeURIComponent(params.subId as string) : ''
  const notebookId = rawSubParam.includes(':')
    ? rawSubParam
    : rawSubParam
    ? `notebook:${rawSubParam}`
    : ''
    ...
    <AppShell>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0">
          ...
          {/* NotebookHeader — identical to original, back link goes to parent folder */}
          <div className="flex-shrink-0 px-3 sm:px-4 pt-3 pb-0">
            <NotebookHeader
              notebook={notebook}
              backHref={`/notebooks/${parentShortId}`}
          ...
          <div className="flex-1 p-3 sm:p-4 pt-3 sm:pt-4 overflow-x-hidden lg:overflow-x-auto flex flex-col min-h-0">
            ...
            {/* Desktop: Collapsible columns layout — identical to original notebook page */}
            <div
              className={cn(
