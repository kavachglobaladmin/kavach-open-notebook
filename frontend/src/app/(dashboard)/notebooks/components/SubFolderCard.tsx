...
export function SubFolderCard({ notebook, parentId, onUnlink }: SubFolderCardProps) {
  const { language } = useTranslation()
  const router = useRouter()

  // Stable color derived from the notebook ID
  const colorIdx =
    Math.abs(
      notebook.id.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0),
    ) % FOLDER_COLORS.length
  const color = FOLDER_COLORS[colorIdx]

  const handleOpen = () => {
    const shortParentId = parentId.includes(':') ? parentId.split(':')[1] : parentId
    const shortChildId = notebook.id.includes(':') ? notebook.id.split(':')[1] : notebook.id
    router.push(`/notebooks/${shortParentId}/sub/${shortChildId}`)
  }
