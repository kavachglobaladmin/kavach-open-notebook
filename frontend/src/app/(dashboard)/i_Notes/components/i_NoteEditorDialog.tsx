...
export function NoteEditorDialog({ open, onOpenChange, i_NotesId, note }: NoteEditorDialogProps) {
  ...
  const onSubmit = async (data: CreateNoteFormData) => {
    if (note) {
      ...
      // Only invalidate i_Notes-specific queries if we have a i_NotesId
      if (i_NotesId) {
        queryClient.invalidateQueries({ queryKey: QUERY_KEYS.notes(i_NotesId) })
      }
    } else {
      // Creating a note requires a i_NotesId
      if (!i_NotesId) {
        console.error('Cannot create note without i_Notes_id')
        return
      }
