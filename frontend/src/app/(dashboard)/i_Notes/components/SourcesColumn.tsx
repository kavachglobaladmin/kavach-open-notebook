...
import { useDeleteSource, useRetrySource, useRemoveSourceFromi_Notes } from '@/lib/hooks/use-sources'
import { ConfirmDialog } from '@/components/common/ConfirmDialog'
import { useModalManager } from '@/lib/hooks/use-modal-manager'
import { ContextMode } from '../[id]/page'
import { CollapsibleColumn, createCollapseButton } from '@/components/i_Notes/CollapsibleColumn'
...
export function SourcesColumn({
  ...
  const { sourcesCollapsed, toggleSources } = usei_NotesColumnsStore()
  ...
  const handleRemoveConfirm = async () => {
    ...
    try {
      await removeFromi_Notes.mutateAsync({ i_NotesId, sourceId: sourceToRemove })
  ...
  return (
    <>
                ...
                {filteredSources.map((source) => (
                  <div key={source.id} data-source-search-hit={normalizedSearchTerm ? 'true' : undefined}>
                    <SourceCard
                      ...
                      onRetry={(id) => retrySource.mutateAsync({ sourceId: id, i_NotesId })}
                      onRemoveFromi_Notes={(id) => { setSourceToRemove(id); setRemoveDialogOpen(true); }}
                      onRefresh={onRefresh}
                      showRemoveFromi_Notes={true}
      ...
      <AddSourceDialog open={addDialogOpen} onOpenChange={setAddDialogOpen} defaulti_NotesId={i_NotesId} onSuccess={onRefresh} />
      <AddExistingSourceDialog open={addExistingDialogOpen} onOpenChange={setAddExistingDialogOpen} i_NotesId={i_NotesId} onSuccess={onRefresh} />
      ...
      <ConfirmDialog open={removeDialogOpen} onOpenChange={setRemoveDialogOpen} title={t.sources.removeFromi_Notes} description={t.sources.removeConfirm} confirmText={t.common.remove} onConfirm={handleRemoveConfirm} isLoading={removeFromi_Notes.isPending} confirmVariant="default" />
