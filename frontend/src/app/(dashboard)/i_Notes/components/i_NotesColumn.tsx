...
import { NoteEditorDialog } from './i_NoteEditorDialog'
...
export function NotesColumn({
  ...
  const { notesCollapsed, toggleNotes } = usei_NotesColumnsStore()
  ...
  return (
    <>
            ...
            {isLoading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center pt-10">
                ...
                <h3 className="text-[16px] font-bold text-slate-900 mb-1">
                  {normalizedSearchTerm ? 'No matching notes' : t.i_Notes.noNotesYet}
                </h3>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setNoteToDelete(note.id); setDeleteDialogOpen(true); }} className="text-red-600">
                              <Trash2 className="h-4 w-4 mr-2" /> {t.i_Notes.deleteNote}
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
      ...
      <NoteEditorDialog open={showAddDialog || Boolean(editingNote)} onOpenChange={(open) => { if (!open) { setShowAddDialog(false); setEditingNote(null); } else setShowAddDialog(true); }} i_NotesId={i_NotesId} note={editingNote ?? undefined} />
      <ConfirmDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} title={t.i_Notes.deleteNote} description={t.i_Notes.deleteNoteConfirm} confirmText={t.common.delete} onConfirm={handleDeleteConfirm} isLoading={deleteNote.isPending} confirmVariant="destructive" />
    </>
  )
}
