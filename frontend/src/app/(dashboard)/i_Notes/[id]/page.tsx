...
export default function i_NotesFolderPage() {
  const { t } = useTranslation()
  const params = useParams()
  const searchParams = useSearchParams()

  const rawParam = params?.id ? decodeURIComponent(params.id as string) : ''
  const i_NotesId = rawParam.includes(':') ? rawParam : rawParam ? `i_Notes:${rawParam}` : ''
  const queryFromUrl = searchParams?.get('q')?.trim() || ''

  const { data: i_Notes, isLoading: i_NotesLoading } = usei_Notes(i_NotesId)
  const chatTitle = 'Chat with ALL i_Notes'

  const { childIds, addChild, removeChild } = useSubFolders(i_NotesId)
  ...
  const contexti_NotesIds = childIds
  const contexti_NotesIdKey = contexti_NotesIds.join('|')

  const { data: alli_Notes } = usei_Notes(false)
  const { data: allArchivedi_Notes } = usei_Notes(true)

  const alli_NotesFlat = useMemo(
    () => [...(alli_Notes ?? []), ...(allArchivedi_Notes ?? [])],
    [alli_Notes, allArchivedi_Notes],
  )
  const subFolders = alli_NotesFlat.filter((nb) => childIds.includes(nb.id))
  const sourcesQueries = useQueries({
    queries: contexti_NotesIds.map((id) => ({
  ...
  const folderContextsStamp = `${contexti_NotesIdKey}::${sourcesDataStamp}::${notesDataStamp}`
  if (folderContextsCacheRef.current.stamp !== folderContextsStamp) {
    // subFolders already has correct names from alli_NotesFlat.
    // Build a lookup: folderId → index in contexti_NotesIds (= childIds).
    const folderIdToIndex = new Map(contexti_NotesIds.map((id, i) => [id, i]))
    ...
    // Fall back to alli_NotesFlat for any folder not yet in subFolders.
    alli_NotesFlat.forEach((nb) => {
      if (!folderNameLookup.has(nb.id)) folderNameLookup.set(nb.id, nb.name)
    })

    folderContextsCacheRef.current = {
      stamp: folderContextsStamp,
      value: contexti_NotesIds.map((folderId) => {
  ...
  const handleSubFolderCreated = (newi_NotesId: string) => {
    addChild(newi_NotesId)
  }
