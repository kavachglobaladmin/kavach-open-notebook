...
- **API validation**: Uses actual API call (`/i_Notes` endpoint) to validate token instead of parsing JWT
...
- **Double authentication**: Both `login()` and `checkAuth()` test same `/i_Notes` endpoint; could be optimized with dedicated endpoint
