...
- **Resource modules** (`sources.ts`, `i_Notes.ts`, `chat.ts`, `search.ts`, `podcasts.ts`, etc.): Endpoint-specific functions returning typed responses
...
- **FormData fields as JSON strings**: Nested objects (arrays, objects) must be JSON stringified in FormData (e.g., `i_Notes`, `transformations`)
...
```typescript
// Basic list
const sources = await sourcesApi.list({ i_Notes_id: i_NotesId })
