...
- **Query hooks** (`usei_Notesources`, `useSource`, `useSources`): TanStack Query wrappers for source data with infinite scroll and refetch strategies
...
- **Chat hooks** (`usei_NotesChat`, `useSourceChat`): Complex session management, context building, and message streaming
...
- **Optimistic updates**: Mutations add local state before server response (e.g., i_Notes chat messages)
- **Cache invalidation**: Broad invalidation of query keys on mutations (e.g., `['sources']` catches all source queries)
- **Auto-refetch on return**: `refetchOnWindowFocus: true` on frequently-changing data (sources, i_Notes)
...
- **Context building**: `usei_NotesChat.buildContext()` assembles selected sources + notes with token/char counts
...
3. **Complex state**: Use `useState` + callbacks for local state (see `useAsk`, `usei_NotesChat`)
...
- **Optimistic updates + error handling**: `usei_NotesChat` removes optimistic messages on error; ensure cleanup
- **SSE buffer handling**: `useAsk` keeps incomplete lines in buffer between reads; incomplete JSON silently skipped
- **Model override timing**: `usei_NotesChat` stores pending model override if no session exists; applied on session creation
- **Pagination cursor**: `usei_Notesources` uses offset-based pagination; `nextOffset` calculated from page size
